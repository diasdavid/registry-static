...
// Check arg types
options.string('hooks user group log report registry index error tmp dir domain'.split(' '))
options.boolean('sync restart check tarballs clone clean version quiet'.split(' '))
...
console.log('SEQ FILE PATH: ', options.seqFile)
...
if (options.blobstore) {
  options.blobstore = require(options.blobstore)(options.dir)
} else {
